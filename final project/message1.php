		...
		<div class="message-left">
			<ul>
				<?php
					...
					$q = mysqli_query($con, "SELECT * FROM `user` WHERE user = '1' or user = '2' or user = '3' or user = '4' and id!='$user_id'");
					//display all the results
					while($row = mysqli_fetch_assoc($q)){
						echo "<a href='message1.php?id={$row['id']}'><li><img src='{$row['profile_pic']}'> {$row['username']}</li></a>";
					}
				?>
			</ul>
		</div>
