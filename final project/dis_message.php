...
<div class="message-body">
		<div class="message-left">
			<ul>
				<?php
					...
					$q = mysqli_query($con, "SELECT * FROM `user` WHERE user = '1' or user = '4' and id!='$user_id'");
					//display all the results
					while($row = mysqli_fetch_assoc($q)){
						echo "<a href='dis_message.php?id={$row['id']}'><li><img src='{$row['profile_pic']}'> {$row['username']}</li></a>";
					}
				?>
			</ul>
		</div>
		...
		<div class="message-right">
			...
			<div class="send-message">
				<!-- distrbutors conversation_id, user_from, user_to so that we can send send this values to post_message_ajax.php -->
